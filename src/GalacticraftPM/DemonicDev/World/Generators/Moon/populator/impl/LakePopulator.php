...
namespace GalacticraftPM\DemonicDev\World\Generators\Moon\populator\impl;

use GalacticraftPM\DemonicDev\World\Generators\Moon\populator\Populator;
use Generator;
...
class LakePopulator extends Populator {
	...
	public function populate(ChunkManager $world, int $chunkX, int $chunkZ, Random $random): void {
		...
		foreach($this->getRandomShape($random) as $vec) {
			...
			$block = $vec->addVector($pos)->getY() < $pos->getY() ? VanillaBlocks::AIR() : VanillaBlocks::AIR();

			$blocks[] = [$finalPos, $block];
			/** @phpstan-ignore-next-line */
			if($block->isSameType(VanillaBlocks::AIR()) && ($world->getBlockAt($finalPos->getX() + 1, $finalPos->getY(), $finalPos->getZ()) || $world->getBlockAt($finalPos->getX() - 1, $finalPos->getY(), $finalPos->getZ()) || $world->getBlockAt($finalPos->getX(), $finalPos->getY(), $finalPos->getZ() + 1) || $world->getBlockAt($finalPos->getX(), $finalPos->getY(), $finalPos->getZ() - 1))) {
				return;
			}
		}
